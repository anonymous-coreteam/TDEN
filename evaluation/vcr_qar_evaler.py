    ...
    def eval(self, model, epoch):
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(self.val_loader):
                ...
                vil_logit = model(
                    input_txt, features, spatials, segment_ids, input_mask, image_mask, 'VCR_QA-R')
                ...
                for i in range(vil_logit.size(0)):
                    ...
                    if np.argmax(ans) == target[i]:
                        accuracy += 1
                        qar_res[question_id[i].item()] = True
                    else:
                        qar_res[question_id[i].item()] = False
        ...
        json.dump(results, open(os.path.join(result_folder, str(epoch) + '_QA_R.json'), 'w'))

        return 'QA -> R: %.3f' % accuracy, qar_res
